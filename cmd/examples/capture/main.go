...
func main() {
	...
	flag.StringVar(&srcFileName, "src", "/dev/video1", "source video")
	flag.Parse()
	...
	dict := gmf.NewDict([]gmf.Pair{
		{Key: "video_size", Val: "1280x1024"},
		//{Key: "video_size", Val: "1280x720"},
		//{Key: "video_size", Val: "800x600"},
		{Key: "framerate", Val: "10"},
	})
	opt := gmf.Option{Key: "input_options", Val: dict}

	if err := inputCtx.SetInputFormat("v4l2"); err != nil {
		log.Fatalf("Error set input format - %s\n", err)
	}
	if err := inputCtx.OpenInputWithOption(srcFileName, &opt); err != nil {
